...
- cloudflare-pages-workers — edge hosting/runtime: Pages, Workers isolates, wrangler, CF Web Analytics, Turnstile.
...
> email transport is **Resend (HTTP email API)**, not Microsoft Graph — the
> domain is on Proton with no M365 tenant. (Cloudflare's native `send_email`
> binding was evaluated and rejected: it is Workers-only and Cloudflare **Pages**
> does not support it; Resend is a plain HTTP call that works on Pages, like the
> fleet's Graph-over-HTTP.) The `microsoft-graph` pairing is dropped. Cert count
> corrected 16 → 14 (actual content count).
...
- Secrets never committed — Turnstile secret key, Resend API key, and CI tokens go through `wrangler pages secret` / CI secret store. Only `PUBLIC_`-prefixed values may be committed as `vars`.
...
### Conversion
- Lead form (SvelteKit form action) with Turnstile verification, sending email via Resend (HTTP API); "Discovery Call" intent.
...
2. Lead form verifies Turnstile and sends a real email via Resend.
